...
| `ai_generator.py` | 封装 Claude API 调用，支持**多轮**工具调用循环（最多 `MAX_TOOL_ROUNDS=2` 轮） |
...
└── rag_system.py:102  RAGSystem.query(query, session_id)
    ...
    └── ai_generator.py:43  AIGenerator.generate_response(query, history, tools, tool_manager)
        ...
        │
        └── [循环 for _ in range(MAX_TOOL_ROUNDS)]
            │
            └── [如果 stop_reason == "tool_use"]
                │
                ├── tool_manager.execute_tool("search_course_content", **kwargs)
                │   │
                │   └── search_tools.py  CourseSearchTool.execute(query, course_name?, lesson_number?)
                │       │
                │       └── vector_store.py  VectorStore.search(query, course_name?, lesson_number?)
                │           │
                │           ├── VectorStore._resolve_course_name()  # 如果有 course_name
                ...
                │
                ├── 把 tool_result 追加到 messages，继续下一轮
                │
                └── [如果预算耗尽] anthropic API 最后一次调用 (不带 tools，强制出文本)
    │
...
### 4. ai_generator.py — generate_response()

构建 Claude API 请求，然后进入**多轮工具循环**（最多 `config.MAX_TOOL_ROUNDS = 2` 轮）。每轮：

1. 把系统提示词、历史对话、当前 messages、工具定义组装成 API 参数，调用 `client.messages.create(...)`
2. 如果 `stop_reason != "tool_use"`，模型已给出文本答案，直接返回
3. 否则遍历 `tool_use` 块，通过 `ToolManager.execute_tool()` 执行，每个结果包成 `tool_result` 块追加到 messages，进入下一轮

轮数耗尽后做一次**不带 `tools`** 的兜底请求，强制 Claude 出文本——避免无限循环，也避免模型因预算耗尽返回空内容。

这样设计是为了让 Claude 在第一次搜索结果不理想时可以改写 query 重试——实测对"搜错课 / query 太模糊 / 需要先查大纲再查内容"的问题有显著改善，详见 `evals/README.md` 改进记录 2026-04-08。
...
- **多轮工具调用**：`config.MAX_TOOL_ROUNDS = 2` 硬上限。Claude 可以在看到第一次搜索结果后改写 query 再搜一次，对模糊 query 有显著帮助
